...
# Helper function to use caching if implemented
async def groq_complete_if_cache(
    ...
    **kwargs
    ...
    # For now, we'll just call groq_complete directly
    return await groq_complete(prompt, system_prompt, history_messages, **kwargs)

# Main function to be used in LightRAG
async def groq_complete_main(
    prompt, system_prompt=None, history_messages=[], **kwargs
) -> str:
    return await groq_complete_if_cache(
        "llama3-8b-8192",
        prompt,
        system_prompt=system_prompt,
        history_messages=history_messages,
